...
## Problematics
The application is able to identify the intended language based on the special characters, so the model will have to take care of the remaining cases where there are no unique characters based on the reliable words previously used by the user. As some strings or similar structures can be encountered in multiple languages, the model should predict many desirably with a lower confidence. The data is gathered from the user input. 
