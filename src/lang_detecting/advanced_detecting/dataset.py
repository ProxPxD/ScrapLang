    ...
    def __init__(self, data: DataFrame, tokenizer: MultiKindTokenizer, conf: Conf, shuffle: bool = True):
        ...
        for length, bucket in len_bucketed:
            ...
            for i in range(0, len(bucket), batch_size):
                ...
                tokenized_outputs = _.map_(outputs, c().map(tokenizer.tokenize_target))
                one_hot_encoded_outputs = torch.zeros(len(tokenized_outputs), tokenizer.n_target_tokens, dtype=torch.float32)
